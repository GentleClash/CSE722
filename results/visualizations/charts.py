...
# ============================================================================
print("\n[1/7] Creating demand generation model plot...")
...
axes[0,0].plot(time_hours[:288*2], daily_cycle[:288*2], linewidth=2.5, color='#2E86AB')
...
axes[0,0].axhline(y=1.0, color='gray', linestyle='--', alpha=0.5, label='Peak')
axes[0,0].axhline(y=0.4, color='gray', linestyle='--', alpha=0.5, label='Trough')
axes[0,0].legend()
...
axes[1,0].hist(noise, bins=50, density=True, alpha=0.7, color='#F18F01', edgecolor='black')
x_noise = np.linspace(-75, 75, 200)
gaussian = (1/(25*np.sqrt(2*np.pi))) * np.exp(-0.5*(x_noise/25)**2)
axes[1,0].plot(x_noise, gaussian, 'r-', linewidth=2.5, label='Theoretical N(0,25)')
...
axes[1,1].set_title('D. Complete Demand Profile (48 hours)', fontweight='bold')
axes[1,1].legend()
...
plt.tight_layout()
plt.savefig(output_dir / 'demand_generation_model.png', dpi=300, bbox_inches='tight')
...
# ============================================================================
print("[2/7] Creating solar generation model plot...")
...
axes[0,0].plot(hours_day, solar_ideal, linewidth=3, color='#FF8C00', label='Clear Sky')
axes[0,0].fill_between(hours_day, 0, solar_ideal, alpha=0.3, color='#FF8C00')
axes[0,0].axvline(x=6, color='purple', linestyle='--', alpha=0.5, label='Sunrise')
axes[0,0].axvline(x=18, color='purple', linestyle='--', alpha=0.5, label='Sunset')
axes[0,0].axvline(x=12, color='red', linestyle='--', alpha=0.5, label='Solar Noon')
...
axes[0,1].axvline(x=1.0, color='red', linestyle='--', linewidth=2, label='Clear Sky')
...
for i in range(10):
    ...
    axes[1,0].plot(hours_day, solar_cloudy, linewidth=1, alpha=0.4, color='gray')
axes[1,0].set_xlabel('Hour of Day', fontweight='bold')
axes[1,0].set_ylabel('Solar Generation (MW)', fontweight='bold')
axes[1,0].set_title('C. Solar Generation with Cloud Variability (10 days)', fontweight='bold')
axes[1,0].legend()
...
axes[1,1].plot(time_hours, solar_7days, linewidth=1.5, color='#FF8C00', alpha=0.8)
...
axes[1,1].set_title('D. Seven-Day Solar Generation Profile', fontweight='bold')

# Add day/night shading
for day in range(7):
    axes[1,1].axvspan(day*24, day*24+6, alpha=0.1, color='black', label='Night' if day==0 else '')
...
plt.tight_layout()
plt.savefig(output_dir / 'solar_generation_model.png', dpi=300, bbox_inches='tight')
...
# ============================================================================
print("[3/7] Creating wind generation model plot...")
...
axes[1,1].plot(time_hours, wind_7days, linewidth=1.5, color='#20B2AA', alpha=0.8)
...
plt.tight_layout()
plt.savefig(output_dir / 'wind_generation_model.png', dpi=300, bbox_inches='tight')
...
# ============================================================================
print("[4/7] Creating net load calculation plot...")
...
plt.tight_layout()
plt.savefig(output_dir / 'net_load_calculation.png', dpi=300, bbox_inches='tight')
...
# ============================================================================
print("[5/7] Creating generator cost functions plot...")
...
plt.tight_layout()
plt.savefig(output_dir / 'generator_cost_functions.png', dpi=300, bbox_inches='tight')
...
# ============================================================================
print("[6/7] Creating merit order analysis plot...")
...
plt.savefig(output_dir / 'merit_order_analysis.png', dpi=300, bbox_inches='tight')
...
# ============================================================================
print("[7/7] Creating complete system overview...")
...
ax1.fill_between(time_hours[:288], 0, complete_demand[:288], alpha=0.3, color='#2C3E50')
...
ax2.fill_between(time_hours[:288], 0, solar_7days[:288], alpha=0.3, color='#FF8C00')
...
bars = ax5.bar(methods, costs, color=colors_bar, alpha=0.7, edgecolor='black', linewidth=2)
ax5.set_ylabel('Total Cost ($)', fontweight='bold')
ax5.set_title('Method Comparison: Total Cost (288 timesteps)', fontweight='bold')
